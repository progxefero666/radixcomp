...
export async function get(id:number): Promise<string> {
    ...
    try {
        ...
        DpOperationUtil.consoleErr(error, DpOperationUtil.getOpName(DB_TABLES.application, "GET_BY_ID"));
        return JsonResponse.ERROR(DpOperationUtil.getErrMessage(error));
    }
    finally {
        await prisma.$disconnect();
    }
    return JsonResponse.SUCCESS(DpOperationUtil.getOpName(DB_TABLES.application,"GET_BY_ID"), result);
...
export async function getAll(table: string): Promise<string> {
    ...
    try {
        ...
        DpOperationUtil.consoleErr(error, DpOperationUtil.getOpName(DB_TABLES.application, "GET_ALL"));
        return JsonResponse.ERROR(DpOperationUtil.getErrMessage(error));
    }
    finally {
        await prisma.$disconnect();
    }
    return JsonResponse.SUCCESS(DpOperationUtil.getOpName(DB_TABLES.application, "GET_ALL"), result);
